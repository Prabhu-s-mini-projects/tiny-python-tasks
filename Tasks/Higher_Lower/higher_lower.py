...
def compare(compare_a: int, compare_b: int, user_input: str) -> bool:
    ...
    return ((compare_a >= compare_b and user_input == 'a')
            or (compare_b >= compare_a and user_input == 'b'))
...
def main()-> None:
    ...
    while continue_game:
        ...
        compare_a = compare_b
        print(f"Compare A : {compare_a.get('name')},"
              f"{compare_a.get('description')},{compare_a.get('country')}.")

        print(db.VS)

        compare_b = random.choice(db.data)
        print(f"Compare B : {compare_b.get('name')},"
              f"{compare_b.get('description')},{compare_b.get('country')}.")

        answer = input("Who has more followers?  Type 'A' or 'B' : \t").lower()

        continue_game = compare(compare_a.get('follower_count'),
                                compare_b.get('follower_count'), answer)
        if continue_game:
            score += 1
            print("Correct :-) !")
        else:
