...
def caesar(text, shift, direction) -> str:
    ...
    #     1. if the current index - shift goes below 0 during decrypt. add 26 (handled using overflow and shift)
    #     2. if the current Index + shift goes above 26 durin encoding sub 26 (handled using overflow and shift)
    """
    cipher_text = ''
    over_flow = 1
    if direction == 'encode':
